...
def split_dataset(kaggle_dataset_id, batch_size=Config.BATCH_SIZE):
    image_paths = load_kaggle_dataset(kaggle_dataset_id)
    np.random.shuffle(image_paths)
    

    train_dataset = InpaintingDataset(
        image_paths, Config.INPUT_SIZE, Config.MASK_SIZE, batch_size, shuffle=True, augment=True, use_black_mask=False
    ).to_tf_dataset()
